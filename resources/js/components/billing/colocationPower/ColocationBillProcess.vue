                  ...
                  <ValidationProvider name="grp_or_zone_id" rules="required|min_value:1" v-slot="{ errors }">
                    <div class="form-group">
                      ...
                      <select class="form-control" style="width: 100%" v-model="invoice.grp_or_zone_id">
                        ...
                        <option :value="item.id" v-for="(item, index) in listData" :key="index" v-if="item.service_id==23">{{ item.group_name }}</option>
                      </select>
                  ...
                  <table class="table table-striped table-bordered table-hover w-100" style="min-width: 100%">
                    <thead class="thead-dark">
                      <tr>
                        ...
                        <th scope="col" title="Power"> Power </th>
                        <th scope="col" title="Power"> Rate + Qty</th>
                        <th scope="col" title="Power">Power Rate</th>
                        <th scope="col" title="Rack">Rack</th>
                        <th scope="col" title="Rack">Rate + Qty</th>
                        <th scope="col" title="Rack">Rack Rate</th>
                    ...
                    <tbody>
                      <tr v-for="(item, index) in demandNoteList" :key="index">
                        ...
                        <td class="dateFillSize">
                          <span v-for="(power, cIndex) in item.total_power" :key="cIndex" class="badge badge-success dateFillSize">{{ power.name }}</span>
                        </td>
                        <td class="dateFillSize">
                          <span v-for="(power, cIndex) in item.total_power" :key="cIndex" class="badge badge-success dateFillSize">{{ power.price }} + {{ power.PowerQty }}</span>
                        </td>

                        <td class="dateFillSize"> {{rateForPower(item.total_power)}}</td>

                        <td class="dateFillSize">
                          <span v-for="(rack, cIndex) in item.total_rack" :key="cIndex" class="badge badge-success dateFillSize">{{ rack.name}}</span>
                        </td>
                        <td class="dateFillSize">
                          <span v-for="(rack, cIndex) in item.total_rack" :key="cIndex" class="badge badge-success dateFillSize">{{ rack.price}}={{ rack.rackQty}}</span>
                        </td>

                         <td class="dateFillSize"> {{rateForRack(item.total_rack)}}</td>
    ...
    methods: {
       ...
       getlandingGroupList() {
        ...
        getcustomerList(
            ...
            axios
                .post("/get/service/group-zone/wise/power/customer", {
       ...
       showCustomerAndMrcInfo() {
        ...
        onSubmit() {
            ...
            axios
                .post("/bill-rocess/colocation/power", {
