  ...
  <div class="content-wrapper">
    <div class="modal-header">
      <h4 class="text-center" style="color: blue">
        <b>View Invoice List (Colocation Power)</b>
      </h4>
      <router-link
        :to="{
              ...
              >
                ...
                <tbody>
                  <tr v-for="(item, index) in demandNoteList" :key="index">
                    ...
                    <td scope="col">{{ item.customers.custype.cus_type_name }}</td>
                    <td scope="col">{{ item.colocation_bill.groups.group_name}}</td>
                    <td scope="col">{{ item.net_total }}</td>
                    <!-- <td v-if="item.invoice_ganarete == 0 && item.invoice_status == 1"> -->
                    <td >
                        <router-link
                          :to="{
...
export default {
  ...
  methods: {
    ...
    getSingleBillInfo(id) {
      axios
        .post("/get/colocation/port/single/bill-info", { id: id})
        .then((res) => {
          this.loading = false;
