...
class DemandNoteAuthController extends Controller
{
    ...
    {
        $data = [];
        $invoiceList = IplcBillDetails::with(
            'customers',
            'iplcbill.subservice',
            'iplcbill.custype',
            'iplcbill.services',
            'iplcbill.groups'
        )->where('invoice_status', 1)->orderBy('id', 'DESC')->get();
    ...
    public function customerIdAndCableWiseTotalBandwidthCalculation($customerId, $serviceId, $subServiceId, $groupId)
    {
        ...
        if ($serviceId == 1) {
            ...
            $total = DemandNote::where('customer_id', $customerId)->where($colName, $var)->where(
                'approval_status',
                2
            )->sum('max');
            $data = DemandNote::where('customer_id', $customerId)->where($colName, $var)->where(
                'approval_status',
                2
            )->sum('downgrade');
